...
from loss.contrast_loss_our import SupConLoss
...
def main(props=None):
    ...
    criterion_pcl = SupConLoss(threshold=args.slice_threshold, temperature=args.temp, contrastive_method=args.contrastive_method_pcl).to(args.device)
    criterion_simclr = SupConLoss(threshold=args.slice_threshold, temperature=args.temp, contrastive_method=args.contrastive_method_simclr).to(args.device)
    optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, weight_decay=1e-5)
    scheduler = LR_Scheduler(args.lr_scheduler, args.lr, args.epochs, len(train_loader))

    for epoch in range(args.epochs):
        # train for one epoch
        train_loss = train(train_loader, model, criterion, criterion_pcl, criterion_simclr, epoch, optimizer, scheduler, logger, args)
...
def train(data_loader, model, criterion, criterion_pcl, criterion_simclr, epoch, optimizer, scheduler, logger, args):
    model.train()
    losses = AverageMeter()
    for batch_idx, tup in enumerate(data_loader):
        ...
        # loss = criterion_pcl(features, labels=slice_position) + criterion_simclr(features)
...
if __name__ == '__main__':
    ...
    # args.classes = 64
    # for i in np.arange(0.05, 0.50, 0.10):
